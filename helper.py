...
def test_fetch_length_features(q1, q2):
    ...
    # Use difflib for longest common substring length
    import difflib
    seq = difflib.SequenceMatcher(None, q1, q2)
    match = seq.find_longest_match(0, len(q1), 0, len(q2))
...
def preprocess(q):
    if not isinstance(q, str):
        q = '' if q is None or q != q else str(q)
    q = str(q).strip()
    q = q.replace('%', ' percent')
    q = q.replace('$', ' dollar ')
    q = q.replace('₹', ' rupee ')
...
def query_point_creator(q1, q2):
    ...
    input_query.append(round(test_common_words(q1, q2) / (total_words if total_words else 1), 2))
    ...
    # Ensure input_query is 22 features; adjust if you add/remove features above
